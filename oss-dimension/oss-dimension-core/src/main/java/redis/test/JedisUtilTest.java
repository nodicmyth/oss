...
public class JedisUtilTest {
    ...
    @Before
    public void setUp() {
        pool = new JedisPool(new JedisPoolConfig(), "192.168.84.46");
    ...
    public void testBasicString() {
        ...
        System.out.println("mset ==== " + jedis.mget("name", "jarorwar"));
    }
    ...
    public void testMap() {
        ...
        System.out.println("names = " + rsmap);
        // 删除map中的某个键值
        jedis.hdel("user", "pwd");
        System.out.println(jedis.hmget("user", "pwd")); // 因为删除了，所以返回的是null

        jedis.hset("user", "pwd", "pass");

        System.out.println("new pass : " + jedis.hmget("user", "pwd"));
    ...
    public void testObject() {
        ...
        request.setDimKey(new Column("store_key", 1l));
        request.setColumns(columns);
        request.addColumn(new Column("first_name", "mike"));
        request.addColumn(new Column("last_name", "Hillyer"));

        Map<String, String> dimInfo = new HashMap<String, String>();
        dimInfo.put(request.toKeyString(), request.toValueString());

        jedis.hmset("dimInfo", dimInfo);

        List<String> rsmap = jedis.hmget("dimInfo", request.toKeyString());
        System.out.println("names = " + rsmap);

        DimensionRequestDTO request1 = new DimensionRequestDTO();
        Set<Column> columns1 = new HashSet<Column>();
        request1.setDimensionName("dim_product");
        request1.setDimKey(new Column("store_key", 1l));
        request1.setColumns(columns1);
        request1.addColumn(new Column("first_name", "mike"));
        request1.addColumn(new Column("last_name", "Hillyer"));

        jedis.hset("dimInfo", request1.toKeyString(), request1.toValueString());

        rsmap = jedis.hmget("dimInfo", request.toKeyString(), request1.toKeyString());
        System.out.println("names = " + rsmap);
...
//        jedis.del("dimInfo");
        ...
        System.out.println(jedis.hget("dimInfo", request1.toKeyString()));// 返回map对象中的所有value [minxr,
